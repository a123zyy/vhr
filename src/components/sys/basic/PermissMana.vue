        ...
        methods: {
            deleteRole(role) {
                this.$confirm('此操作将永久删除【' + role.nameZh + '】角色, 是否继续?', '提示', {
                    ...
                    client.delete("/system/basic/permiss/role/" + role.id).then(resp => {
                        if (resp) {
                            this.initRoles();
                        }
                    })
            ...
            doAddRole() {
                if (this.role.name && this.role.nameZh) {
                    ...
                    client.post("/system/basic/permiss/role", this.role).then(resp => {
            ...
            initSelectedMenus(rid) {
                client.get("/system/basic/permiss/mids/" + rid).then(resp => {
                    if (resp) {
            ...
            initAllMenus() {
                client.get("/system/basic/permiss/menus").then(resp => {
                    if (resp) {
            ...
            initRoles() {
                this.loading = true;
                client.get("/system/basic/permiss/").then(resp => {
                     this.loading = false;
                    if (resp) {
