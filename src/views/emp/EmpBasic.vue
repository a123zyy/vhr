        ...
        methods: {
            ...
            deleteEmp(data) {
                this.$confirm('此操作将永久删除【' + data.name + '】, 是否继续?', '提示', {
                    ...
                    client.delete("/employee/basic/" + data.id).then(resp => {
                        if (resp) {
                            this.initEmps();
                        }
                    })
            ...
            doAddEmp() {
                if (this.emp.id) {
                    this.$refs['empForm'].validate(valid => {
                        if (valid) {
                            client.patch("/employee/basic/", this.emp).then(resp => {
                                if (resp) {
                    ...
                    this.$refs['empForm'].validate(valid => {
                        if (valid) {
                            client.put("/employee/basic/", this.emp).then(resp => {
                                if (resp) {
                                    this.dialogVisible = false;
                                    this.initEmps();
                                }
            ...
            initPositions() {
                client.get('/employee/basic/positions').then(resp => {
                    if (resp) {
            ...
            getMaxWordID() {
                client.get("/employee/basic/maxWorkID").then(resp => {
                    if (resp) {
            ...
            initData() {
                if (!window.sessionStorage.getItem("nations")) {
                    client.get('/employee/basic/nations').then(resp => {
                        if (resp) {
                            console.log(resp)
                ...
                if (!window.sessionStorage.getItem("joblevels")) {
                    client.get('/employee/basic/joblevels').then(resp => {
                        if (resp) {
                ...
                if (!window.sessionStorage.getItem("politicsstatus")) {
                    client.get('/employee/basic/politicsstatus').then(resp => {
                        if (resp) {
                ...
                if (!window.sessionStorage.getItem("deps")) {
                    client.get('/employee/basic/deps').then(resp => {
                        if (resp) {
            ...
            initEmps(type) {
                ...
                const Employee = {
                    pageNo:this.page,
                    pageSize:this.size,
                    politicId:this.searchValue.politicId,
