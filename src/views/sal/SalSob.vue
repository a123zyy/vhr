        ...
        methods: {
            ...
            deleteSalary(data) {
                this.$confirm('此操作将删除【' + data.name + '】账套，是否继续？', '提示', {
                    ...
                    client.delete("/salary/sob/" + data.id).then(resp => {
                        if (resp) {
                            this.initSalaries();
                        }
                    })
            ...
            nextStep() {
                if (this.activeItemIndex == 10) {
                    if (this.salary.id) {
                        ...
                        client.put("/salary/sob/", this.salary).then(resp=>{
                            if (resp) {
                                this.initSalaries();
                                this.dialogVisible = false;
                            }
                        ...
                        this.salary.createDate = new Date();
                        client.patch("/salary/sob/", this.salary).then(resp => {
                            if (resp) {
                                this.initSalaries();
                                this.dialogVisible = false;
                            }
            ...
            initSalaries() {
                client.get("/salary/sob/").then(resp => {
                    if (resp) {
